       ...
       PROGRAM-ID. ORD815R.
      **********************************************  Z-WIN-RPG2   ****
      * PROGRAM ORD815 AV ESPEN LARSEN 30/01-1991                     *
      * LESER SEQ ORDREFILE OG DANNER ORDRESUM RECORDS.               *
      * 27/10-92  LAGT INN RUTINE FOR Å SKILLE UT                     *
      *           INTERN ORDRE.                                       *
      *  6/02-95  LAGT INN RUTINE FOR Å REGISTRERE FEILPLUKK          *
      *           PR. PLUKKER.                                        *
      *  6/03-96  ORDRE MED F,T,R I BK SKAL IKKE MED.                 *
      *           DETTE ER FORHÅNDSORDRE, TILBUDSORDRE OG RESTORDRE.  *
      * 18/02-97  SUM ANT. LEVERT OG IKKE ANTALL BESTILT.             *
      *           LAGEROVERFØRING MERKES MED "L" I RUTINE-IDENT.      *
      *           FORHÅNDSORDRE BLIR IKKE TATT MED.                   *
      * 26.02.98  ORDREMOTAGER FIRMA ER OGSÅ TATT MED (HELLA,BOSCH,MF)*
      * 03.12.04  ORDREMOTAGER INITALER I EGEN FELT PÅ DIR.ORDRE.     *
      * 27.11.06  veng vil ha med i8 i initialer                      *
      * 04.12.12  505555 ER IKKE INTERNKUNDENR                        *
      *****************************************************************
      *
      ...
      *
       ...
       01  WORK-AREA-BATCH.
           ...
           05  TEMPORARY-FIELDS.
               ...
               10  ANTVFP-IO.
                   15  ANTVFP              PICTURE S9(5).
               10  FEILPL                  PICTURE X(2).
               10  ANTVL-IO.
               ...
               10  KOSTP-IO.
                   15  KOSTP               PICTURE S9(7)V9(2).
               10  ORDSUM-IO.
                   15  ORDSUM              PICTURE S9(9)V9(2).
               10  KOSSUM-IO.
                   15  KOSSUM              PICTURE S9(9)V9(2).
               10  NETANT-IO.
      ...
      *  ALLE FIRMA BLIR NÅ SELEKTERT 17.01.2002.                     *
      *****************************************************************
           END-IF
           IF  (I-01)
               SET I-50                    TO TRUE
      *****************************************************************
      * KREDITNOTA MED BONUS SKAL IKKE VÆRE MED FOR ENKELTE FIRMA     *
      *****************************************************************
           END-IF
           IF  (I-01)
      ...
      *****************************************************************
           ...
           IF  (I-01 AND I-96)
               ...
               IF  ORDMOT = 'I8'
                   SET I-97                TO TRUE
               END-IF
      *  01 96 97          SETOF                         11 SKAL MED STA
           END-IF
           IF  (I-01)
               MOVE 1                      TO ANTORD
               PERFORM INTORD-S
       ...
       SLUTT-T.
      *****************************************************************
      *  SUBRUTINE FOR Å SE OM ORDEREN ER EN INTERN ORDRE.            *
      *  ORDREMOTAGER BLIR DA SATT TIL "QQ"                           *
      *****************************************************************
           CONTINUE.
      ...
      *****************************************************************
       ...
       INRUT4-S-P.
           ...
           END-IF
           IF  (I-91)
               ADD ANTBES                  TO ANTVFP
               MOVE FEILPA                 TO FEILPL
           END-IF
           IF  (NOT-I-24 AND NOT-I-91)
               ADD 1                       TO ANTVL
           END-IF
           ...
           IF  ANTBES > ANTLEV
               SET I-12                    TO TRUE
           END-IF
           IF  (NOT-I-24 AND I-12 AND NOT-I-91)
               ADD 1                       TO ANTVLR
           END-IF.
      *****************************************************************
      ...
      *  SNU BELØP DERSOM KREDIT-GEBYR ELLER ANNEN RETUR.
           ...
           MULTIPLY ANTLEV BY KOSPRI   GIVING KOSTP ROUNDED
           IF  (I-98)
               MULTIPLY -1 BY KOSTP    GIVING KOSTP
           END-IF
           IF  (NOT-I-23)
               ADD NETPRI                  TO ORDSUM
           END-IF
           IF  (I-23)
               SUBTRACT NETPRI             FROM ORDSUM
           END-IF
           IF  (NOT-I-23)
               ADD KOSTP                   TO KOSSUM
           END-IF
           IF  (I-23)
               SUBTRACT KOSTP              FROM KOSSUM
           END-IF
      ...
      *****************************************************************
       ...
       TOTAL-OUTPUT-P.
           IF  (I-L1 AND I-50)
               MOVE SPACES TO ORDSUMR-IO-AREA
               INITIALIZE ORDSUMR-IO-AREA
               MOVE 'A'                    TO ORDSUMR-IO-AREA (1:1)
               MOVE FIRMA                  TO ORDSUMR-IO-AREA (2:3)
               MOVE ORDNR                  TO ORDSUMR-IO-AREA (5:6)
               MOVE ORDATO-IO              TO ORDSUMR-IO-AREA (11:6)
               MOVE ORDMOT                 TO ORDSUMR-IO-AREA (17:2)
               IF  (I-11)
                   MOVE '**'               TO ORDSUMR-IO-AREA (17:2)
               END-IF
               IF  (I-55)
                   MOVE 'QQ'               TO ORDSUMR-IO-AREA (17:2)
               END-IF
               MOVE KUNDNR                 TO ORDSUMR-IO-AREA (19:6)
               MOVE STATUS-X               TO ORDSUMR-IO-AREA (25:1)
               MOVE RUTID                  TO ORDSUMR-IO-AREA (26:1)
               IF  (I-48)
                   MOVE 'L'                TO ORDSUMR-IO-AREA (26:1)
               END-IF
               MOVE KRETYP                 TO ORDSUMR-IO-AREA (27:1)
               MOVE FRITT                  TO ORDSUMR-IO-AREA (28:1)
               MOVE LAGER                  TO ORDSUMR-IO-AREA (29:2)
               MOVE BK                     TO ORDSUMR-IO-AREA (31:1)
               MOVE BETM                   TO ORDSUMR-IO-AREA (32:2)
               MOVE GEBYR                  TO ORDSUMR-IO-AREA (34:1)
               MOVE AVD                    TO ORDSUMR-IO-AREA (35:1)
               MOVE REST                   TO ORDSUMR-IO-AREA (36:1)
               MOVE TERMID                 TO ORDSUMR-IO-AREA (37:4)
               MOVE SELGKP                 TO ORDSUMR-IO-AREA (41:1)
               MOVE HND                    TO ORDSUMR-IO-AREA (42:3)
               MOVE PAKKER                 TO ORDSUMR-IO-AREA (45:2)
               MOVE FERDIM                 TO ORDSUMR-IO-AREA (47:1)
               IF  (I-11)
                   MOVE ORDMOT             TO ORDSUMR-IO-AREA (48:2)
               END-IF
               MOVE ANTORD                 TO XO-70P
               MOVE XO-70P-EF              TO ORDSUMR-IO-AREA (51:4)
               MOVE ORDSUM                 TO XO-92P
               MOVE XO-92P-EF              TO ORDSUMR-IO-AREA (55:6)
               INITIALIZE ORDSUM
               MOVE KOSSUM                 TO XO-92P
               MOVE XO-92P-EF              TO ORDSUMR-IO-AREA (61:6)
               INITIALIZE KOSSUM
               MOVE RESSUM                 TO XO-92P
               MOVE XO-92P-EF              TO ORDSUMR-IO-AREA (67:6)
               INITIALIZE RESSUM
               MOVE ANTVL                  TO XO-90P
               MOVE XO-90P-EF              TO ORDSUMR-IO-AREA (73:5)
               INITIALIZE ANTVL
               MOVE ANTVLR                 TO XO-90P
               MOVE XO-90P-EF              TO ORDSUMR-IO-AREA (78:5)
               INITIALIZE ANTVLR
               MOVE ANTVFP                 TO XO-50P
               MOVE XO-50P-EF              TO ORDSUMR-IO-AREA (83:3)
               INITIALIZE ANTVFP
               MOVE FEILPL                 TO ORDSUMR-IO-AREA (86:2)
               INITIALIZE FEILPL
               WRITE ORDSUMR-IO-AREA
           END-IF.
