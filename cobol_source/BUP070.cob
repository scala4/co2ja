       ...
       PROGRAM-ID. BUP070R.
      **********************************************  Z-WIN-RPG2   ****
      *                  ERSTATTER BUP071        ***TXT***OK MT       *
      *  PROGRAM.......: BUP070 (ERSTATTER PROGRAM BUP071).           *
      *                  PLUKKER UT BOKF-TRANSER.                     *
      *  PROGRAMMERER..: MORTEN TUVRØNNINGEN                          *
      *  KJØRES I JOBB.: BUP20UD.                                     *
      *  LAGET DATO....: 20.09.95                                     *
      *  RETTET........: 06.03.96 SJEKKER OM BILAGET ER GODKJENT, TAR *
      *                           UT IKKE GODKJENTE BILAG.            *
      *  INPUT.........: REGNSKAP.BOKF.TRANSER (REGTRA2).             *
      *  BEHANDLING....: LESER HEADING OG DELTALJ TRANS-RECORD (ART A)*
      *                  FRA REGISTRERING I BOKF, OG LEGGER UT TIL    *
      *                  INPUT I DOP12UD.                             *
      *  OUTPUT........: KOPI.BOKF.TRANSER (BILAGSART 4-8)            *
      *****************************************************************
      *
      ...
      *
       ...
       FILE-CONTROL.
           SELECT BOKFTRA
               ASSIGN TO UT-S-BOKFTRA
               ACCESS MODE IS SEQUENTIAL
               ORGANIZATION IS SEQUENTIAL
               STATUS IS BOKFTRA-STATUS.
           SELECT KOPI
               ASSIGN TO UT-S-KOPI
               ACCESS MODE IS SEQUENTIAL
               ORGANIZATION IS SEQUENTIAL
       ...
       FD BOKFTRA
               ...
               RECORD CONTAINS 300.
       01  BOKFTRA-IO-AREA.
           05  BOKFTRA-IO-AREA-X           PICTURE X(300).
       FD KOPI
               BLOCK CONTAINS 2400
               RECORD CONTAINS 240.
       01  KOPI-IO-AREA.
       ...
       01  FILE-STATUS-TABLE.
           10  BOKFTRA-STATUS              PICTURE 99 VALUE 0.
           10  KOPI-STATUS                 PICTURE 99 VALUE 0.
           10  LISTE-STATUS                PICTURE 99 VALUE 0.
 
       01  WORK-AREA-BATCH.
           05  FILLER                      PIC X VALUE '0'.
               88  BOKFTRA-EOF-OFF         VALUE '0'.
               88  BOKFTRA-EOF             VALUE '1'.
           05  FILLER                      PIC X VALUE '0'.
               88  BOKFTRA-READ-OFF        VALUE '0'.
               88  BOKFTRA-READ            VALUE '1'.
           05  FILLER                      PIC X VALUE '0'.
               88  BOKFTRA-PROCESS-OFF     VALUE '0'.
               88  BOKFTRA-PROCESS         VALUE '1'.
           ...
           05  BOKFTRA-DATA-FIELDS.
               10  KEYREC                  PICTURE X(60).
               10  TRAREC                  PICTURE X(240).
               10  TREC                    PICTURE X(100).
       ...
       MAINLINE-LOOP.
           ...
           IF  BOKFTRA-PROCESS
               SET BOKFTRA-PROCESS-OFF     TO TRUE
               SET BOKFTRA-READ            TO TRUE
           END-IF
 
           IF  BOKFTRA-READ
           AND RECORD-SELECTED-OFF
               PERFORM BOKFTRA-GET
               SET BOKFTRA-READ-OFF        TO TRUE
               IF  NOT BOKFTRA-EOF
                   SET BOKFTRA-PROCESS     TO TRUE
       ...
       BOKFTRA-GET-P.
           IF  BOKFTRA-EOF-OFF
               READ BOKFTRA
               AT END
                   SET BOKFTRA-EOF         TO TRUE
                   SUBTRACT 1            FROM LR-CHECK
               END-READ
           END-IF.
 
       BOKFTRA-FLDSET SECTION.
       BOKFTRA-FLDSET-P.
           EVALUATE TRUE
           WHEN ( BOKFTRA-IO-AREA (61:1) = '1' )
               MOVE BOKFTRA-IO-AREA (1:60) TO KEYREC (1:60)
               MOVE BOKFTRA-IO-AREA (61:240) TO TRAREC (1:240)
               MOVE BOKFTRA-IO-AREA (61:100) TO TREC (1:100)
               MOVE BOKFTRA-IO-AREA (56:1) TO OKBIL (1:1)
               MOVE BOKFTRA-IO-AREA (82:10) TO BILTOT-IO
               INSPECT BILTOT-IO REPLACING ALL ' ' BY '0'
               MOVE BOKFTRA-IO-AREA (92:1) TO DEBKRE (1:1)
           WHEN ( BOKFTRA-IO-AREA (61:1) = '2' )
               MOVE BOKFTRA-IO-AREA (1:60) TO KEYREC (1:60)
               MOVE BOKFTRA-IO-AREA (61:240) TO TRAREC (1:240)
               MOVE BOKFTRA-IO-AREA (61:100) TO TREC (1:100)
           END-EVALUATE.
 
       BOKFTRA-IDSET SECTION.
       BOKFTRA-IDSET-P.
           EVALUATE TRUE
           WHEN ( BOKFTRA-IO-AREA (61:1) = '1' )
               SET I-01                    TO TRUE
       ...
       DETAIL-OUTPUT-P.
           ...
           IF  (I-01 AND I-11)
               MOVE SPACES TO LISTE-IO-AREA
               INITIALIZE LISTE-IO-AREA
               MOVE KEYREC                 TO LISTE-IO-AREA (1:60)
               ...
               MOVE SPACES TO LISTE-IO-AREA
               INITIALIZE LISTE-IO-AREA
               MOVE TREC                   TO LISTE-IO-AREA (1:100)
       ...
       TOTAL-OUTPUT-P.
           IF  (I-LR)
               ...
               MOVE 'REG BILAGSART 4 - 8     ' TO LISTE-IO-AREA (33:24)
               MOVE '(BOKF)'               TO LISTE-IO-AREA (56:6)
               MOVE 2                      TO LISTE-AFTER-SPACE
               PERFORM LISTE-PRINT-LINE
               MOVE SPACES TO LISTE-IO-AREA
               INITIALIZE LISTE-IO-AREA
               ...
               MOVE 'BUP070'               TO LISTE-IO-AREA (33:6)
               MOVE 2                      TO LISTE-AFTER-SPACE
               PERFORM LISTE-PRINT-LINE
           END-IF.
       ...
       INITIALIZATION-P.
           ...
           MOVE 1                          TO LR-CHECK
           INITIALIZE BOKFTRA-DATA-FIELDS
           SET BOKFTRA-EOF-OFF             TO TRUE
           SET BOKFTRA-PROCESS             TO TRUE
           OPEN INPUT BOKFTRA
           OPEN OUTPUT KOPI
           OPEN OUTPUT LISTE
