      ...
      *
       ...
       FILE-CONTROL.
           ...
           SELECT RABINN
               ASSIGN TO UT-S-RABINN
               ACCESS MODE IS SEQUENTIAL
               ORGANIZATION IS SEQUENTIAL
               STATUS IS RABINN-STATUS.
           SELECT RABMAST
               ASSIGN TO RABMAST
       ...
       FD RABINN
               ...
               RECORD CONTAINS 40.
       01  RABINN-IO-AREA.
           05  RABINN-IO-AREA-X            PICTURE X(40).
       FD RABMAST
               RECORD CONTAINS 40.
       01  RABMAST-IO-AREA.
       ...
       01  FILE-STATUS-TABLE.
           ...
           10  RABINN-STATUS               PICTURE 99 VALUE 0.
       ...
       01  WORK-AREA-BATCH.
           ...
           05  FILLER                      PIC X VALUE '0'.
               88  RABINN-EOF-OFF          VALUE '0'.
               88  RABINN-EOF              VALUE '1'.
           05  FILLER                      PIC X VALUE '0'.
               88  RABINN-READ-OFF         VALUE '0'.
               88  RABINN-READ             VALUE '1'.
           05  FILLER                      PIC X VALUE '0'.
               88  RABINN-PROCESS-OFF      VALUE '0'.
               88  RABINN-PROCESS          VALUE '1'.
           05  RABMAST-KEY-NUM             PICTURE 9 VALUE 1 BINARY.
           05  PARAM-DATA-FIELDS.
               10  FRAFNR                  PICTURE X(3).
               10  TILFNR                  PICTURE X(3).
           05  RABINN-DATA-FIELDS.
       ...
       MAINLINE-LOOP.
           ...
           IF  RABINN-PROCESS
               SET RABINN-PROCESS-OFF      TO TRUE
               SET RABINN-READ             TO TRUE
           END-IF
 
           IF  RABINN-READ
           AND RECORD-SELECTED-OFF
               PERFORM RABINN-GET
               SET RABINN-READ-OFF         TO TRUE
               IF  NOT RABINN-EOF
                   SET RABINN-PROCESS      TO TRUE
                   SET RECORD-SELECTED     TO TRUE
               END-IF
           END-IF
       ...
       RABINN-GET-P.
           IF  RABINN-EOF-OFF
               READ RABINN
               AT END
                   SET RABINN-EOF          TO TRUE
                   SUBTRACT 1            FROM LR-CHECK
               END-READ
           END-IF.
 
       RABINN-FLDSET SECTION.
       RABINN-FLDSET-P.
           EVALUATE TRUE
           WHEN ANY
               MOVE RABINN-IO-AREA (2:3)   TO FIRMA (1:3)
               MOVE RABINN-IO-AREA (2:20)  TO KEY-X (1:20)
               MOVE RABINN-IO-AREA (1:40)  TO RREC (1:40)
           END-EVALUATE.
 
       RABINN-IDSET SECTION.
       RABINN-IDSET-P.
           SET I-04                        TO TRUE.
       ...
       DETAIL-OUTPUT-P.
           IF  (I-04 AND I-11 AND NOT-I-20)
               MOVE RREC                   TO RABMAST-IO-AREA (1:40)
               MOVE TILFNR                 TO RABMAST-IO-AREA (2:3)
               REWRITE RABMAST-IO-AREA
                   INVALID KEY
                       DISPLAY 'Bad REWRITE - file = RABMAST'
               END-REWRITE
           END-IF
           IF  (I-04 AND I-11 AND I-20)
               ...
               WRITE RABMAST-IO-AREA
                   INVALID KEY
                       DISPLAY 'Bad WRITE - file = RABMAST'
               END-WRITE
           END-IF.
       ...
       INITIALIZATION-P.
           ...
           SET PARAM-PROCESS               TO TRUE
           OPEN INPUT PARAM
           INITIALIZE RABINN-DATA-FIELDS
           SET RABINN-EOF-OFF              TO TRUE
           SET RABINN-PROCESS              TO TRUE
           OPEN INPUT RABINN
           OPEN I-O RABMAST.
