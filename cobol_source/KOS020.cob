       ...
       PROGRAM-ID. KOS020R.
      **********************************************  Z-WIN-RPG2   ****
      *  PROGRAM.......: KOS020, SANERING AV KONTANT.BETALING.KODER   *
      *  PROGRAMMERER..: MORTEN TUVRØNNINGEN                          *
      *  KJØRES I JOBB.: KONTBK, REORG. AV KONTANT.BETALING.KODER     *
      *  LAGET DATO....: 19.02.04                                     *
      *  ENDRET........:                                              *
      *****************************************************************
      *
      ...
      *
       ...
       FILE-CONTROL.
           SELECT KONTBKI
               ASSIGN TO UT-S-KONTBKI
               ACCESS MODE IS SEQUENTIAL
               ORGANIZATION IS SEQUENTIAL
               STATUS IS KONTBKI-STATUS.
           SELECT FIRMAKT
           ...
           SELECT KONTBKO
               ASSIGN TO UT-S-KONTBKO
               ACCESS MODE IS SEQUENTIAL
               ORGANIZATION IS SEQUENTIAL
               STATUS IS KONTBKO-STATUS.
           SELECT KLISTEO
       ...
       FD KONTBKI
               ...
               RECORD CONTAINS 20.
       01  KONTBKI-IO-AREA.
           05  KONTBKI-IO-AREA-X           PICTURE X(20).
       FD FIRMAKT
               BLOCK CONTAINS 40
               RECORD CONTAINS 4.
       01  FIRMAKT-IO-AREA.
       ...
       FD KONTBKO
               ...
               RECORD CONTAINS 20.
       01  KONTBKO-IO-AREA.
           05  KONTBKO-IO-AREA-X           PICTURE X(20).
      *BUGFILO O   F  80  80            PRINTERSYSLST
       ...
       01  FILE-STATUS-TABLE.
           10  KONTBKI-STATUS              PICTURE 99 VALUE 0.
           10  FIRMAKT-STATUS              PICTURE 99 VALUE 0.
           10  KONTBKO-STATUS              PICTURE 99 VALUE 0.
           10  KLISTEO-STATUS              PICTURE 99 VALUE 0.
 
       01  WORK-AREA-BATCH.
           05  FILLER                      PIC X VALUE '0'.
               88  KONTBKI-EOF-OFF         VALUE '0'.
               88  KONTBKI-EOF             VALUE '1'.
           05  FILLER                      PIC X VALUE '0'.
               88  KONTBKI-READ-OFF        VALUE '0'.
               88  KONTBKI-READ            VALUE '1'.
           05  FILLER                      PIC X VALUE '0'.
               88  KONTBKI-PROCESS-OFF     VALUE '0'.
               88  KONTBKI-PROCESS         VALUE '1'.
           05  FILLER                      PIC X VALUE '0'.
           ...
           05  KONTBKI-DATA-FIELDS.
               10  FIRKEY                  PICTURE X(3).
               10  REC020                  PICTURE X(20).
           05  KONTBKI-MP                  PICTURE X(3).
           05  KONTBKI-MC                  PICTURE X(3).
           05  KONTBKI-M-01            REDEFINES KONTBKI-MC.
               10  KONTBKI-M-01-M1.
                   15  KONTBKI-M-01-M1-FIRKEY-G.
                       20  KONTBKI-M-01-M1-FIRKEY PICTURE X(3).
       ...
       MAINLINE-LOOP.
           ...
           IF  KONTBKI-PROCESS
               SET KONTBKI-PROCESS-OFF     TO TRUE
               SET KONTBKI-READ            TO TRUE
           END-IF
 
           IF  KONTBKI-READ
               PERFORM KONTBKI-GET
               SET KONTBKI-READ-OFF        TO TRUE
               IF  NOT KONTBKI-EOF
                   PERFORM KONTBKI-MATCH-SET
               END-IF
           END-IF
      ...
      *  01NMR   BUGFL1    DEBUGBUGFILO   REC120           VIS FELT/IND
       ...
       KONTBKI-GET-P.
           IF  KONTBKI-EOF-OFF
               READ KONTBKI
               AT END
                   SET KONTBKI-EOF         TO TRUE
                   SUBTRACT 1            FROM LR-CHECK
               END-READ
           END-IF.
 
       KONTBKI-FLDSET SECTION.
       KONTBKI-FLDSET-P.
           EVALUATE TRUE
           WHEN ANY
               MOVE KONTBKI-IO-AREA (1:3)  TO FIRKEY (1:3)
               MOVE KONTBKI-IO-AREA (1:20) TO REC020 (1:20)
           END-EVALUATE.
 
       KONTBKI-IDSET SECTION.
       KONTBKI-IDSET-P.
           SET I-01                        TO TRUE.
 
       KONTBKI-MATCH-SET SECTION.
       KONTBKI-MATCH-SET-P.
           SET CALL-MATCH-RECS             TO TRUE
           EVALUATE TRUE
           WHEN ANY
               MOVE KONTBKI-IO-AREA (1:3)  TO KONTBKI-M-01-M1-FIRKEY
           END-EVALUATE.
       ...
       MATCHING-RECORDS-P.
           IF  KONTBKI-EOF
               MOVE HIGH-VALUES            TO KONTBKI-MC
                                              KONTBKI-MP
           END-IF
           IF  FIRMAKT-EOF
           ...
           END-IF
           EVALUATE TRUE
           WHEN  KONTBKI-MC < FIRMAKT-MC
               IF  RECORD-SELECTED-OFF
                   SET RECORD-SELECTED     TO TRUE
                   SET KONTBKI-PROCESS     TO TRUE
                   MOVE KONTBKI-MC         TO KONTBKI-MP
                   IF  KONTBKI-MC = FIRMAKT-MP
                       SET SET-I-MR        TO TRUE
                   ELSE
                       SET NOT-SET-I-MR    TO TRUE
                   END-IF
               END-IF
           WHEN  FIRMAKT-MC < KONTBKI-MC
               IF  RECORD-SELECTED-OFF
                   SET RECORD-SELECTED     TO TRUE
                   SET FIRMAKT-PROCESS     TO TRUE
                   MOVE FIRMAKT-MC         TO FIRMAKT-MP
                   IF  FIRMAKT-MC = KONTBKI-MP
                       SET SET-I-MR        TO TRUE
                   ELSE
                       SET NOT-SET-I-MR    TO TRUE
                   END-IF
       ...
       DETAIL-OUTPUT-P.
           IF  (I-01 AND I-MR)
               MOVE SPACES TO KONTBKO-IO-AREA
               INITIALIZE KONTBKO-IO-AREA
               MOVE REC020                 TO KONTBKO-IO-AREA (1:20)
               WRITE KONTBKO-IO-AREA
           END-IF
           IF  (I-01 AND NOT-I-MR AND I-U2)
               MOVE SPACES TO KLISTEO-IO-AREA
               INITIALIZE KLISTEO-IO-AREA
               MOVE 'SANERT:'              TO KLISTEO-IO-AREA (1:7)
               MOVE REC020                 TO KLISTEO-IO-AREA (11:20)
               IF  I-U1
       ...
       HEADING-OUTPUT-P.
           IF  (I-1P)
               ...
               MOVE 'KONTROLL-LISTE SANERING ' TO KLISTEO-IO-AREA
                                                                (1:24)
               MOVE 'AV KONTANT.BETALING.KODE' TO KLISTEO-IO-AREA
                                                               (25:24)
               MOVE 'R'                    TO KLISTEO-IO-AREA (49:1)
               MOVE 'DATO:'                TO KLISTEO-IO-AREA (53:5)
               MOVE UDATE                  TO EDIT-DATE
               MOVE EDIT-DATE (7:8)        TO KLISTEO-IO-AREA (58:8)
               MOVE 'JOB=KONTBK '          TO KLISTEO-IO-AREA (95:11)
               MOVE 'PROGRAM=KOS020'       TO KLISTEO-IO-AREA (107:14)
               IF  I-U1
       ...
       INITIALIZATION-P.
           ...
           MOVE 2                          TO LR-CHECK
           INITIALIZE KONTBKI-DATA-FIELDS
           SET KONTBKI-EOF-OFF             TO TRUE
           SET KONTBKI-PROCESS             TO TRUE
           MOVE LOW-VALUES                 TO KONTBKI-MC
                                              KONTBKI-MP
           OPEN INPUT KONTBKI
