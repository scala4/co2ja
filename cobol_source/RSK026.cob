       ...
       PROGRAM-ID. RSK026R.
      **********************************************  Z-WIN-RPG2   ****
      *  PROGRAM.......: RSK026, OPPDATERING AV REGFILE.240           *
      *  ENDR 02.10.12.: SKRIVER FIL MED AVSTEMMINGSTALL              *
      *  ENDR 16.10.12.: BRUKER BELØP FRA GAMMELT BELØP TIL NYTT FELT *
      *
      ...
      *
       ...
       FD GMLREG
               ...
               RECORD CONTAINS 240.
       01  GMLREG-IO-AREA.
           05  GMLREG-IO-AREA-X            PICTURE X(240).
       FD DAGREG
               BLOCK CONTAINS 240
               RECORD CONTAINS 240.
       01  DAGREG-IO-AREA.
      ...
      *BUGFILO O   F  80  80            PRINTERSYSLST
       ...
       01  WORK-AREA-BATCH.
           ...
           05  GMLREG-DATA-FIELDS.
               ...
               10  RA1                     PICTURE X(2).
               10  BEL1-IO.
                   15  BEL1                PICTURE S9(11)V9(2) USAGE
                                                       PACKED-DECIMAL.
               10  REGN1                   PICTURE X(1).
               10  VAL1-IO.
                   15  VAL1                PICTURE S9(9)V9(2) USAGE
                                                       PACKED-DECIMAL.
               10  VAL1U-IO.
           ...
           05  DAGREG-DATA-FIELDS.
               ...
               10  RA2                     PICTURE X(2).
               10  BEL2-IO.
                   15  BEL2                PICTURE S9(11)V9(2) USAGE
                                                       PACKED-DECIMAL.
               10  TEGN2                   PICTURE X(1).
               10  VAL2-IO.
                   15  VAL2                PICTURE S9(9)V9(2) USAGE
                                                       PACKED-DECIMAL.
               10  VAL2U-IO.
           ...
           05  TEMPORARY-FIELDS.
               ...
               10  TOTVAL-IO.
                   15  TOTVAL              PICTURE S9(9)V9(2).
               10  VALTOT-IO.
                   15  VALTOT              PICTURE S9(9)V9(2).
               10  LRRGAN-IO.
           ...
           05  EDITTING-FIELDS.
               ...
               10  EDIT-TOTVAL             PICTURE ZZZ.ZZZ.ZZZ,ZZ-.
               10  EDIT-VALTOT             PICTURE ZZZ.ZZZ.ZZZ,ZZ-.
       01  WORK-AREA.
           05  INDICATOR-TABLE.
               COPY TCRPGIN.
      ...
      *  02                MOVE "REC2    "BUGFL1  8        DISPLAY FIELD
      *  02      BUGFL1    DEBUGBUGFILO   REC2             VIS INDIKATOR
           END-IF
           IF  (I-01)
               ADD 1                       TO TOTREC
           END-IF
           ...
           END-IF
           IF  (I-01 AND NOT-I-10)
               SUBTRACT VAL1               FROM TOTVAL
           END-IF
           IF  (I-02 AND I-11)
               ADD VAL2                    TO TOTVAL
           END-IF
           IF  (I-02 AND NOT-I-11)
               SUBTRACT VAL2               FROM TOTVAL
           END-IF
           IF  (I-01 AND I-10)
               ADD VAL1U                   TO VALTOT
           END-IF
           IF  (I-01 AND NOT-I-10)
               SUBTRACT VAL1U              FROM VALTOT
           END-IF
           IF  (I-02 AND I-11)
               ADD VAL2U                   TO VALTOT
           END-IF
           IF  (I-02 AND NOT-I-11)
               SUBTRACT VAL2U              FROM VALTOT
           END-IF.
       ...
       GMLREG-FLDSET-P.
           EVALUATE TRUE
           WHEN ANY
               MOVE GMLREG-IO-AREA (1:240) TO REC1 (1:240)
               MOVE GMLREG-IO-AREA (1:2)   TO RA1 (1:2)
               MOVE GMLREG-IO-AREA (121:7) TO BEL1-IO
               MOVE GMLREG-IO-AREA (32:1)  TO REGN1 (1:1)
               IF  REGN1 = SPACES
                   SET I-10                TO TRUE
               END-IF
               MOVE GMLREG-IO-AREA (33:6)  TO VAL1-IO
               MOVE GMLREG-IO-AREA (128:8) TO VAL1U-IO
           END-EVALUATE.
       ...
       DAGREG-FLDSET-P.
           EVALUATE TRUE
           WHEN ANY
               MOVE DAGREG-IO-AREA (1:240) TO REC2 (1:240)
               MOVE DAGREG-IO-AREA (1:2)   TO RA2 (1:2)
               MOVE DAGREG-IO-AREA (121:7) TO BEL2-IO
               MOVE DAGREG-IO-AREA (32:1)  TO TEGN2 (1:1)
               IF  TEGN2 = SPACES
                   SET I-11                TO TRUE
               END-IF
               MOVE DAGREG-IO-AREA (33:6)  TO VAL2-IO
               MOVE DAGREG-IO-AREA (128:8) TO VAL2U-IO
           END-EVALUATE.
       ...
       DETAIL-OUTPUT-P.
           IF  (I-01)
               ...
               MOVE REC1                   TO NYREG-IO-AREA (1:240)
               WRITE NYREG-IO-AREA
           END-IF
           IF  (I-02)
               MOVE SPACES TO NYREG-IO-AREA
               INITIALIZE NYREG-IO-AREA
               MOVE REC2                   TO NYREG-IO-AREA (1:240)
               WRITE NYREG-IO-AREA
           END-IF.
       ...
       TOTAL-OUTPUT-P.
           IF  (I-LR)
               ...
               MOVE 'RSK026'               TO AVSTEMM-IO-AREA (15:6)
               MOVE '*RSK026*'             TO AVSTEMM-IO-AREA (21:8)
           ...
           IF  (I-LR)
               MOVE SPACES TO LISTE-IO-AREA
               INITIALIZE LISTE-IO-AREA
               MOVE 'AVSTEMMING PROG. RSK026 ' TO LISTE-IO-AREA (1:24)
               ...
               MOVE SPACES TO LISTE-IO-AREA
               INITIALIZE LISTE-IO-AREA
               MOVE 'VALUTA1 NY REGFILE'   TO LISTE-IO-AREA (3:18)
               MOVE TOTVAL                 TO EDIT-TOTVAL
               MOVE EDIT-TOTVAL            TO LISTE-IO-AREA (22:15)
               INITIALIZE TOTVAL
               MOVE 2                      TO LISTE-AFTER-SPACE
               PERFORM LISTE-PRINT-LINE
               MOVE SPACES TO LISTE-IO-AREA
               INITIALIZE LISTE-IO-AREA
               MOVE 'VALUTA2 NY REGFILE'   TO LISTE-IO-AREA (3:18)
               MOVE VALTOT                 TO EDIT-VALTOT
               MOVE EDIT-VALTOT            TO LISTE-IO-AREA (22:15)
               INITIALIZE VALTOT
               MOVE 2                      TO LISTE-AFTER-SPACE
               PERFORM LISTE-PRINT-LINE
           END-IF.
