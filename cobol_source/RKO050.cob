       ...
       PROGRAM-ID. RKO050R.
      **********************************************  Z-WIN-RPG2P     *
      * PROGRAM.......: kopi av rko083- FAK12E/RESMEK3.               *
      * ENDR..........: xx.xx.xx                                      *
      *****************************************************************
      *
      ...
      *****************************************************************
       ...
       01  WORK-AREA-BATCH.
           ...
           05  FILLER                      PIC X VALUE '0'.
               88  RESKOMT-EOF-OFF         VALUE '0'.
               88  RESKOMT-EOF             VALUE '1'.
           05  FILLER                      PIC X VALUE '0'.
               88  RESKOMT-READ-OFF        VALUE '0'.
               88  RESKOMT-READ            VALUE '1'.
           05  FILLER                      PIC X VALUE '0'.
               88  RESKOMT-PROCESS-OFF     VALUE '0'.
               88  RESKOMT-PROCESS         VALUE '1'.
           05  FILLER                      PIC X VALUE '1'.
               88  RESKOMT-LEVEL-INIT-OFF  VALUE '0'.
               88  RESKOMT-LEVEL-INIT      VALUE '1'.
           05  CONSOLE-IO-AREA.
      ...
      ******************************************************************
           05  RESKOMT-LEVEL-02.
               10  RESKOMT-02-L1.
                   15  RESKOMT-02-L1-RESKEY PICTURE X(9).
           05  RESKOMT-DATA-FIELDS.
      ...
      *                                      78  78 ONLINE
       ...
       MAINLINE-LOOP.
           ...
           IF  RESKOMT-PROCESS
               SET RESKOMT-PROCESS-OFF     TO TRUE
               SET RESKOMT-READ            TO TRUE
           END-IF
 
           IF  RESKOMT-READ
           AND RECORD-SELECTED-OFF
               PERFORM RESKOMT-GET
               SET RESKOMT-READ-OFF        TO TRUE
               IF  NOT RESKOMT-EOF
                   PERFORM RESKOMT-IDCHK
                   PERFORM HALT-INDICATOR-CHECK
                   SET RESKOMT-PROCESS     TO TRUE
                   SET RECORD-SELECTED     TO TRUE
               END-IF
           END-IF
           ...
           IF  RESKOMT-PROCESS
               PERFORM RESKOMT-IDSET
           END-IF
 
           IF  RESKOMT-PROCESS
               PERFORM RESKOMT-CHK-LEVEL
           END-IF
      ...
      *****************************************************************
       ...
       RESKOMT-GET-P.
           IF  RESKOMT-EOF-OFF
               READ RESKOMT
               AT END
                   SET RESKOMT-EOF         TO TRUE
                   SUBTRACT 1            FROM LR-CHECK
               END-READ
           END-IF.
 
       RESKOMT-FLDSET SECTION.
       RESKOMT-FLDSET-P.
           EVALUATE TRUE
           WHEN ( RESKOMT-IO-AREA (1:1) = '0'
            AND   RESKOMT-IO-AREA (2:1) = '2' )
               MOVE RESKOMT-IO-AREA (1:200) TO RREC (1:200)
               MOVE RESKOMT-IO-AREA (1:60) TO REC60 (1:60)
               MOVE RESKOMT-IO-AREA (24:6) TO RBDATO (1:6)
               MOVE RESKOMT-IO-AREA (42:6) TO RFDATO (1:6)
               MOVE RESKOMT-IO-AREA (48:9) TO RBEL-IO
               INSPECT RBEL-IO REPLACING ALL ' ' BY '0'
               MOVE RESKOMT-IO-AREA (60:1) TO RPA-ELGINK (1:1)
               MOVE RESKOMT-IO-AREA (61:2) TO BETM (1:2)
               MOVE RESKOMT-IO-AREA (3:9)  TO RESKEY (1:9)
           END-EVALUATE.
 
       RESKOMT-IDCHK SECTION.
       RESKOMT-IDCHK-P.
           EVALUATE TRUE
           WHEN ( RESKOMT-IO-AREA (1:1) = '0'
            AND   RESKOMT-IO-AREA (2:1) = '2' )
       ...
       RESKOMT-CHK-LEVEL SECTION.
       RESKOMT-CHK-LEVEL-P.
           EVALUATE TRUE
           WHEN ( RESKOMT-IO-AREA (1:1) = '0'
            AND   RESKOMT-IO-AREA (2:1) = '2' )
               MOVE LOW-VALUES             TO RESKOMT-LEVEL-02
               MOVE RESKOMT-IO-AREA (3:9)  TO RESKOMT-02-L1-RESKEY
               IF  RESKOMT-LEVEL-INIT
                   EVALUATE TRUE
                   WHEN  RESKOMT-02-L1 NOT = THE-PRIOR-L1
                       PERFORM SETON-I-L1
                   END-EVALUATE
               END-IF
               MOVE  RESKOMT-02-L1         TO THE-PRIOR-L1
               SET RESKOMT-LEVEL-INIT      TO TRUE
           END-EVALUATE.
      ...
      *****************************************************************
       ...
       INITIALIZATION-P.
           ...
           OPEN INPUT RESPAR
           SET RESKOMT-LEVEL-INIT          TO TRUE
           INITIALIZE RESKOMT-DATA-FIELDS
           SET RESKOMT-EOF-OFF             TO TRUE
           SET RESKOMT-PROCESS             TO TRUE
           OPEN INPUT RESKOMT
           OPEN OUTPUT SALDO
           OPEN OUTPUT INKASSO.
