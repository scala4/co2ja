       ...
       PROGRAM-ID. RKO278R.
      **********************************************  Z-WIN-RPG2   ****
      * PROGRAM   : RKO278 - FJERNER RESK.NR SOM ER UTLIGNET          *
      * LAGET AV  : MORTEN TUVRØNNINGEN JULI 2013                     *
      * E XX.XX.XX:                                                   *
      *****************************************************************
      *
      ...
      *BUGFILO O   F  80  80            PRINTERSYSLST
       ...
       01  WORK-AREA-BATCH.
           ...
           05  FILLER                      PIC X VALUE '0'.
               88  RSKNULL-EOF-OFF         VALUE '0'.
               88  RSKNULL-EOF             VALUE '1'.
           05  FILLER                      PIC X VALUE '0'.
               88  RSKNULL-READ-OFF        VALUE '0'.
               88  RSKNULL-READ            VALUE '1'.
           05  FILLER                      PIC X VALUE '0'.
               88  RSKNULL-PROCESS-OFF     VALUE '0'.
               88  RSKNULL-PROCESS         VALUE '1'.
           05  FILLER                      PIC X VALUE '1'.
               88  RSKNULL-LEVEL-INIT-OFF  VALUE '0'.
               88  RSKNULL-LEVEL-INIT      VALUE '1'.
           ...
           05  RESFILI-MP                  PICTURE X(9).
           05  RESFILI-MC                  PICTURE X(9).
           ...
           05  RSKNULL-LEVEL-02.
               10  RSKNULL-02-L3.
                   15  RSKNULL-02-L3-FIRMA PICTURE X(3).
               10  RSKNULL-02-L2.
                   15  RSKNULL-02-L2-RESKNR PICTURE X(6).
           05  RSKNULL-DATA-FIELDS.
               10  FILLER                  PICTURE X.
           05  RSKNULL-MP                  PICTURE X(9).
           05  RSKNULL-MC                  PICTURE X(9).
           05  RSKNULL-M-02            REDEFINES RSKNULL-MC.
               10  RSKNULL-M-02-M3.
                   15  RSKNULL-M-02-M3-FIRMA-G.
                       20  RSKNULL-M-02-M3-FIRMA PICTURE X(3).
               10  RSKNULL-M-02-M2.
                   15  RSKNULL-M-02-M2-RESKNR-G.
                       20  RSKNULL-M-02-M2-RESKNR PICTURE X(6).
           05  THE-PRIOR-LEVEL.
       ...
       MAINLINE-LOOP.
           ...
           IF  RSKNULL-PROCESS
               SET RSKNULL-PROCESS-OFF     TO TRUE
               SET RSKNULL-READ            TO TRUE
           END-IF
 
           IF  RSKNULL-READ
               PERFORM RSKNULL-GET
               SET RSKNULL-READ-OFF        TO TRUE
               IF  NOT RSKNULL-EOF
                   PERFORM RSKNULL-MATCH-SET
               END-IF
           END-IF
      ...
      *
       ...
       RSKNULL-GET-P.
           IF  RSKNULL-EOF-OFF
               READ RSKNULL
               AT END
                   SET RSKNULL-EOF         TO TRUE
               END-READ
           END-IF.
 
       RSKNULL-IDSET SECTION.
       RSKNULL-IDSET-P.
           SET I-02                        TO TRUE.
 
       RSKNULL-CHK-LEVEL SECTION.
       RSKNULL-CHK-LEVEL-P.
           EVALUATE TRUE
           WHEN ANY
               MOVE LOW-VALUES             TO RSKNULL-LEVEL-02
               MOVE RSKNULL-IO-AREA (1:3)  TO RSKNULL-02-L3-FIRMA
               MOVE RSKNULL-IO-AREA (4:6)  TO RSKNULL-02-L2-RESKNR
               IF  RSKNULL-LEVEL-INIT
                   EVALUATE TRUE
                   WHEN  RSKNULL-02-L3 NOT = THE-PRIOR-L3
                       PERFORM SETON-I-L3
                   WHEN  RSKNULL-02-L2 NOT = THE-PRIOR-L2
                       PERFORM SETON-I-L2
                   END-EVALUATE
               END-IF
               MOVE  RSKNULL-02-L3         TO THE-PRIOR-L3
               MOVE  RSKNULL-02-L2         TO THE-PRIOR-L2
               SET RSKNULL-LEVEL-INIT      TO TRUE
           END-EVALUATE.
 
       RSKNULL-MATCH-SET SECTION.
       RSKNULL-MATCH-SET-P.
           SET CALL-MATCH-RECS             TO TRUE
           EVALUATE TRUE
           WHEN ANY
               MOVE RSKNULL-IO-AREA (1:3)  TO RSKNULL-M-02-M3-FIRMA
               MOVE RSKNULL-IO-AREA (4:6)  TO RSKNULL-M-02-M2-RESKNR
           END-EVALUATE.
       ...
       MATCHING-RECORDS-P.
           ...
           END-IF
           IF  RSKNULL-EOF
               MOVE HIGH-VALUES            TO RSKNULL-MC
                                              RSKNULL-MP
           END-IF
           IF  RESFILI-MC < RESFILI-MP
               ...
               MOVE 'B'                    TO E-R-R-O-R
           END-IF
           IF  RSKNULL-MC < RSKNULL-MP
               SET I-H0                    TO TRUE
               MOVE 'B'                    TO E-R-R-O-R
           END-IF
           EVALUATE TRUE
           WHEN  RESFILI-MC < RSKNULL-MC
               IF  RECORD-SELECTED-OFF
                   SET RECORD-SELECTED     TO TRUE
                   SET RESFILI-PROCESS     TO TRUE
                   MOVE RESFILI-MC         TO RESFILI-MP
                   IF  RESFILI-MC = RSKNULL-MP
                       SET SET-I-MR        TO TRUE
                   ELSE
                       SET NOT-SET-I-MR    TO TRUE
                   END-IF
               END-IF
           WHEN  RSKNULL-MC < RESFILI-MC
               IF  RECORD-SELECTED-OFF
                   SET RECORD-SELECTED     TO TRUE
                   SET RSKNULL-PROCESS     TO TRUE
                   MOVE RSKNULL-MC         TO RSKNULL-MP
                   IF  RSKNULL-MC = RESFILI-MP
                       SET SET-I-MR        TO TRUE
                   ELSE
                       SET NOT-SET-I-MR    TO TRUE
                   END-IF
       ...
       HEADING-OUTPUT-P.
           IF  (I-97)
               ...
               MOVE 'SPESIFIKASJON"""""""""""""""""""""     
                                                          OLSEI-RA(52)
               MOVE 'PROGRAM RKO278'       TO LISTE-IO-AREA (48:14)
               ...
               MOVE SPACES TO LISTE-IO-AREA
               INITIALIZE LISTE-IO-AREA
               MOVE 'FJERNER RESKNR SOM GÅR I' TO LISTE-IO-AREA (1:24)
       ...
       TOTAL-OUTPUT-P.
           ...
           IF  (I-LR)
               ...
               END-IF
               MOVE '* ALLE TRANSER'       TO LISTE-IO-AREA (78:14)
               IF  I-U6
                   MOVE 1                  TO LISTE-AFTER-SPACE
                   PERFORM LISTE-PRINT-LINE
               END-IF
               ...
               IF  (I-02)
                   MOVE ' '                TO LISTE-IO-AREA (80:1)
               END-IF
               MOVE '* IKKE FJERNET'       TO LISTE-IO-AREA (78:14)
               IF  I-U6
                   MOVE 1                  TO LISTE-AFTER-SPACE
                   PERFORM LISTE-PRINT-LINE
               END-IF
       ...
       INITIALIZATION-P.
           ...
           OPEN INPUT RESFILI
           SET RSKNULL-LEVEL-INIT          TO TRUE
           SET RSKNULL-EOF-OFF             TO TRUE
           SET RSKNULL-PROCESS             TO TRUE
           MOVE LOW-VALUES                 TO RSKNULL-MC
                                              RSKNULL-MP
           OPEN INPUT RSKNULL
           OPEN OUTPUT RESFILO
           IF I-U6
